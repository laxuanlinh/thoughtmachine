...
## Overview
- It's recommended not to use Core API to do reconciliation as it's not performant, instead we can use streaming API
- For example when a new account is created, an event is produced and we can use this event to know that this account has been moved successfully from legacy to Vault Core
## End to end postings migration API lifecycle
- When data is migrated from `Legacy systems` to Vault core, events from `Data Loader API` and `Core streaming API` are streamed to `Data lake`
- These events are then used by `Reconciliation tool` to reconcile with `Legacy systems`
- `Create resource batch response`: acknowledges the request to create resources, `rejected` or `provisioned`. However this doesn't mean the resources have been created successfully in Vault.
- `Resource migrated events`: when all resources are loaded, this is the single point to reconcile what are sent to Vault and what are inserted, success or failure
- `PostingInstructionBatchCreatedEvent`: when a posting instruction batch is accepted
- `AccountBalanceEvent`: contains the balances and all posting instruction batches that changed those balances, used to check if balances are correct
- `AccountUpdatedEvent`: checks if account is not only created but also activated
- Check `Documentation hub` for more events
## Troubleshooting
- If `Kafka error`, likely resources fail to connect to `Data Loader Kafka Topic`, make sure to have `retry`, if the issue persists then talk to Infra team
- If there are messages in `DLQ`, most likely the `structure` of messages are `incorrect`, check the messages against `Migration Data Dictionary` in `Documentation Hub` and resubmit
- If resource batches are rejected, most likely duplicate IDs, check the IDs and resubmit
- If fails to insert new resources into Vault, most likely because the new resources `violate validation rule`, check the resources against `Migration Data Dictionary` in `Documentation Hub` and resubmit
- If an expected message is `missing` from streaming events, check if the resource/resource batch has been `submitted correctly`, if it's but no message then contact Thought Machine
- If a `PIB message` is streamed with `UNKNOWN` status, check that message for details 

# Migration strategies

## Vault migration strategies
- When migrating, we have to make decisions on how to migrate postings and accounts
- For postings, there are 3 options:
  - Load the entire historic postings, the starting balance is the sum of all postings
  - Load some of the historic postings, the remainders are loaded as `starting position`
  - Load a single posting which represents the balance at the point of migration
- Pre-load:
  - Load the majority of data ahead of cutover event
  - Load only the a delta position shortly before the cutover
  - Load all data just ahead of cutover
- Accounts:
  - All open and closed accounts
  - Just open accounts

## Considerations when choosing strategies
- Regulatory requirements:
  - Local regulator may require high oversight
  - Local regulator only requires mid oversight and plays the role of passive observer, only involes in critical decision
  - Local regulator is not a stakeholder and only involes if there is an impact on customers
- Bank size:
  - Tier 1: significant migration experience in-house, can choose more technical options such as parallel run
  - Tier 2/3: has some experience in migration but not end-to-end, might have expertise in some areas but not end-to-end
  - Tier 4/5: has no experience and has to rely on partners 
- Risk appetite:
  - Mitigating risks drives the migration and actively change the approach to mitigate risks
  - Balance between risks and rewards
  - Banks who are willing to take risks to avoid operational and technical costs

## Simplify migrations
- `Isolate drop off` points and `write off deliquent accounts` to reduce the number of accounts to migrate
- `Product hamornization`: large number of subly different products can be expensive, instead we can move customers to the `same product` to minimize the number of `Smart Contracts`, avoid replicating current products, countries can have rules about combining products
- `Minimize history`: for example wallets, current accounts and saving accounts have little use of historic data
- Migrate the correct customer/account base: isolate `active` and `valuable` accounts from `inactive` and `dormant` accounts then remove them, we can also remove accounts with low/negligible balances

## Parallel run migration
- Step 1: `Legacy` and `Vault Core` run `parallely`
- Step 2: All requests still go to `Legacy` as `usual`
- Step 3: Then we use an `ETL pipeline` to migrate data into Vault, depends on the capabilities, the Legacy is still live when the pipeline runs
- Step 4: After that, we have `Event Recociliation` step where data from above is `reconciled` with the `Legacy` system
- Step 5: Then when `new postings` or `new accounts` are created in the Legacy system, they are `streamed` to `Vault core` to `sync up`
